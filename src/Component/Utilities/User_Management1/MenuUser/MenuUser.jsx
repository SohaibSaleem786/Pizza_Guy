...
const MenuUser = () => {
  ...
  function fetchDataForUserId(userId) {
    const apiUrl = 'https://crystalsolutions.com.pk/malikspicy/get_usrmenu.php';
    const data = {
      userid: userId,
    };
    const formData = new URLSearchParams(data).toString();
  ...
  function Update_Menu(user) {
    return new Promise((resolve, reject) => {
      const apiUrl = 'https://crystalsolutions.com.pk/malikspicy/update_permission.php';
      const data = {
        userid: user.id,
        mcode: user.mcode,
        permission: user.permission,
  ...
  return (
    <>
     <Header/>
    <PathHead
        pageName="Utilities > UserManagement > Menu"
        screen="Get_Item"
    ...
    <div className="col-12" style={{ color: 'black' }}>
      ...
      <div className="tab-content" style={{ marginLeft: "16%", marginRight: "3%" }}>
        {[1, 2, 3, 4].map((tabNumber) => (
          activeTab === tabNumber && (
            <div key={tabNumber} style={{ textAlign: 'center', fontSize: '20px' }}>
              <div style={{ height: '330px', overflowY: 'auto', fontSize: '12px', width: '70%', overflowX: 'auto' }}>
                <table className="table table-striped table-bordered table-sm responsive">
                  <thead>
                    <tr>
                      {data.columns.map((column, index) => (
