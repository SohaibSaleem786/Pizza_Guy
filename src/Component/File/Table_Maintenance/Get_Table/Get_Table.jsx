...
const Get_Table = () => {
  ...
  const filteredData = tabledata?.data
    ? tabledata.data.filter((row) =>
        row.t_name.toLowerCase().includes(filterValue.toLowerCase())
      )
    : [];

  const handleRowClick = (item) => {
    if (selectedRow === item.t_id) {
      // If the clicked row is already selected, navigate to the update screen
      navigate(`/Update_Table/${item.t_id}`);
    } else {
      // Set the selectedRow state to the clicked row id
      setSelectedRow(item.t_id);
    }
  };
  return (
    <>
      ...
      <PathHead
        pageName="File > Table Maintenance"
        screen="Get_Item"
        pageLink="/MainPage"
      />
              ...
              >
                ...
                <MDBTableBody>
                  {filteredData.map((item) => (
                    <tr key={item.t_id} onClick={() => handleRowClick(item)}>
                      <td>{item.t_id}</td>
