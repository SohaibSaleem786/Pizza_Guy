...
import { fetchLocation } from "../../../../Redux/Action";
...
const Get_Location = () => {
  ...
  const handleRowClick = (item) => {
    if (selectedRow === item.loid) {
      // If the clicked row is already selected, navigate to the update screen
      navigate(`/Update_Location/${item.loid}`);
    } else {
      // Set the selectedRow state to the clicked row id
      setSelectedRow(item.loid);
    }
  };

  const filteredData = locationdata?.data
    ? locationdata.data.filter((row) =>
        row.lodsc.toLowerCase().includes(filterValue.toLowerCase())
      )
    : [];
  ...
  return (
    <>
      ...
      <PathHead
        pageName="File > Location Maintenance"
        screen="Get_Item"
        pageLink="/MainPage"
      />
      <div className="col-12" style={{ fontFamily: fontFamily }}>
        ...
        <div>
          <div
            className="col-12 Location-container"
            style={{
              padding: "10px",
